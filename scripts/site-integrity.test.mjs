...
for (const file of ["index.html", "post.html", "comments.html"]) {
  ...
  assert.match(html, /assets\/music-player\.js\?v=7/, `${file} should load versioned music player script`);
}
...
assert.match(musicJs, /fixed:\s*true/, "music player should use APlayer fixed mode");
assert.match(musicJs, /mini:\s*true/, "music player should use APlayer mini mode");
