...
assert.match(musicWorkerJs, /NETEASE_COOKIE|fetchNetease/, "music Worker should support Netease cookie or direct API calls");
assert.match(musicWorkerJs, /weapiEncrypt/, "music Worker should implement Netease WEAPI encryption");
assert.match(musicWorkerJs, /\/api\/music\/playlist/, "music Worker should serve the same music proxy route");
assert.match(musicWorkerJs, /\/api\/music\/url/, "music Worker should keep lazy audio URL route");
assert.match(musicWorkerJs, /\/api\/music\/audio/, "music Worker should expose R2-backed audio proxy route");
assert.match(musicWorkerJs, /\/api\/music\/lyric/, "music Worker should lazy-proxy lyrics per track");
...
assert.match(musicWorkerJs, /site-music-cache\.local\/url/, "music Worker should edge-cache audio URL lookup results");
assert.match(musicWorkerJs, /cachedResponse\(request,\s*monthSeconds/, "music Worker should cache lyrics for about one month");
...
assert.match(musicWorkerJs, /buildNeteasePlaylist/, "music Worker should build playlist from Netease API");
assert.match(musicWorkerJs, /data\.code !== 200/, "music Worker should check Netease API response code");
assert.doesNotMatch(musicWorkerJs, /mk_[A-Za-z0-9_-]+|sid=[A-Za-z0-9_-]+/, "music Worker source should not contain hardcoded secrets");
...
assert.match(musicJs, /lrcType:\s*lyricsEnabled \? 3 : 0/, "music player should lazy-load lyrics through APlayer async lyric mode");
assert.match(musicJs, /buildAudioUrl/, "music player should lazy-load audio only when APlayer needs a track");
assert.match(musicJs, /replaceEndpointPath\("\/audio"\)/, "music player should stream tracks through the same-origin audio proxy");
assert.match(musicJs, /buildLyricUrl/, "music player should lazy-load lyrics per current track");
