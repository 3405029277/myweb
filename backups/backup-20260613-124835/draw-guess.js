(function () {
  ...
  let fullscreenViewportHandler = null;
  ...
  function addEvents() {
    ...
    document.addEventListener("fullscreenchange", () => {
      if (!document.fullscreenElement && document.body.classList.contains("drawerFullscreenMode")) exitFullscreenCanvas(true);
      setTimeout(renderCanvas, 80);
    });
    window.addEventListener("orientationchange", () => {
      if (document.body.classList.contains("drawerFullscreenMode")) syncFullscreenViewport();
    });
  ...
  function isCurrentDrawer() { return ws && ws.readyState === WebSocket.OPEN && clientId === currentDrawerId; }
  ...
  function enterFullscreenCanvas() {
    if (!isCurrentDrawer()) return showToast("只有画画的人才能全屏画板", "bad");
    document.body.classList.add("drawerFullscreenMode");
    els.canvasPanel.classList.add("fullscreenCanvas");
    ...
    fullscreenPromise.then(lockLandscape).finally(() => {
      syncFullscreenViewport();
      setTimeout(syncFullscreenViewport, 160);
      setTimeout(renderCanvas, 160);
    });
  }

  function exitFullscreenCanvas(skipBrowserExit) {
    document.body.classList.remove("drawerFullscreenMode");
    els.canvasPanel.classList.remove("fullscreenCanvas", "rotatedFullscreen");
    unbindFullscreenViewport();
    unlockOrientation();
    clearFullscreenViewportVars();
    if (!skipBrowserExit && document.fullscreenElement && document.exitFullscreen) document.exitFullscreen().catch(() => {});
  ...
  function bindFullscreenViewport() {
    if (fullscreenViewportHandler) return;
    fullscreenViewportHandler = () => syncFullscreenViewport();
    if (window.visualViewport) {
      window.visualViewport.addEventListener("resize", fullscreenViewportHandler);
      window.visualViewport.addEventListener("scroll", fullscreenViewportHandler);
    }
    window.addEventListener("resize", fullscreenViewportHandler);
  }

  function unbindFullscreenViewport() {
    if (!fullscreenViewportHandler) return;
    if (window.visualViewport) {
      window.visualViewport.removeEventListener("resize", fullscreenViewportHandler);
      window.visualViewport.removeEventListener("scroll", fullscreenViewportHandler);
    }
    window.removeEventListener("resize", fullscreenViewportHandler);
    fullscreenViewportHandler = null;
  }

  function syncFullscreenViewport() {
    if (!document.body.classList.contains("drawerFullscreenMode")) return;
    ...
    els.canvasPanel.classList.toggle("rotatedFullscreen", width < height);
  }

  function clearFullscreenViewportVars() {
