(function () {
  ...
  function renderMagazineList(posts) {
    ...
    el.innerHTML = posts.map(function (p, index) {
      ...
      return '<article class="magazine-card"><div class="magazine-cover"><div class="cover-mark">' + safeHtml(coverMark) + '</div><div class="cover-kicker">' + safeHtml(p.category || 'Article') + '</div><div class="cover-date">' + safeHtml(formatDate(p.date) || '未标注日期') + '</div><div class="cover-line">' + safeHtml(coverLine) + '</div></div><div class="magazine-body"><h3><a class="stretched-link" href="post.html?slug=' + safeHtml(p.slug) + '">' + safeHtml(p.title) + '</a></h3><p class="desc">' + safeHtml(p.description || '') + '</p><div class="magazine-meta"><span>' + safeHtml(formatDate(p.date)) + '</span><div class="tag-list">' + tags + '</div></div></div></article>';
    }).join('');
  }
  ...
  function buildToc(contentEl, tocEl) {
    ...
    function highlightToc() {
      ...
      tocLinks.forEach(function (a) {
        ...
        a.style.borderBottomColor = isActive ? 'var(--accent-2)' : 'transparent';
      });
    }
  ...
  function setupPageLoader(loadPromise) {
    ...
    var minDelay = reduceMotion ? 0 : 520;
    var targetDelay = reduceMotion ? 0 : 760;
    Promise.all([
      Promise.resolve(loadPromise).catch(function () {}),
      new Promise(function (resolve) { window.setTimeout(resolve, minDelay); })
    ]).then(function () {
      var elapsed = Date.now() - startTime;
      var remain = Math.max(0, targetDelay - elapsed);
      window.setTimeout(function () {
