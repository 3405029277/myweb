...
for (const file of ["index.html", "post.html", "comments.html"]) {
  ...
  assert.match(html, /assets\/music-player\.js\?v=6/, `${file} should load versioned music player script`);
}
...
assert.match(musicCss, /\.site-music-player\s*\{[\s\S]*left:\s*0/, "music player should dock on the left edge");
assert.match(musicCss, /transform:\s*translateX\(calc\(-100% \+ 42px\)\)/, "music player should hide off-screen horizontally");
